...
INPUT_ROOT  = Path(r"D:\college\4-2 mine\manogna\data final headlines")   # contains 2017, 2018, ...
OUTPUT_ROOT = Path(r"D:\college\4-2 mine\manogna\Crude_Bert_Headlines")    # NEW folder to write results
...
def load_crudebert(model_folder: Path, device: str = "cpu"):
    ...
    # Load config -> create model -> load weights
...
def main():
    ...
    for fpath in tqdm(files, desc="Processing files", unit="file"):
        try:
            ...
            out_path = (OUTPUT_ROOT / rel).with_suffix(".xlsx")  # always output xlsx
