...
TRAIN_PATH = r"D:\ACADS\4-2\FIN SOP\Data\Code\Models\Price_train_17-23.csv"   # can be .csv/.xlsx/.xls
TEST_PATH  = r"D:\ACADS\4-2\FIN SOP\Data\Code\Models\Price_test_24-25.csv"    # can be .csv/.xlsx/.xls

OUTDIR = r"D:\ACADS\4-2\FIN SOP\Data\Code\Models\targets_out"
OUTPUT_FORMAT = "csv"  # "csv" or "xlsx"

# Column names in your OHLC files (match exactly)
DATE_COL  = "Date"
OPEN_COL  = "Open"
HIGH_COL  = "High"
LOW_COL   = "Low"
...
# Jump-risk hyperparameters (Parkinson-only definition)
M_LOOKBACK = 60   # m
K_THRESHOLD = 3.0 # k

# ============================================================
# (Do not edit below unless you’re modifying logic)
# ============================================================
...
def add_targets_independent(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Parse & sort by date (important for lag definitions)
    ...
    # --------------------------------------------------------
    # 1) LOG return target (fixed)
    # y_ret[t] = ln(C_t) - ln(C_{t-1})
    # --------------------------------------------------------
    # Safety: Close must be > 0 for log
    close_pos = out[CLOSE_COL].where(out[CLOSE_COL] > 0)
    out["target_return"] = np.log(close_pos) - np.log(close_pos.shift(1))

    # --------------------------------------------------------
    # 2) Parkinson volatility target
    # sigma^2 = (1/(4 ln 2)) * (ln(H/L))^2
    # y_vol = sqrt(sigma^2)
    # --------------------------------------------------------
    # Safety: High/Low must be > 0
    ...
    # --------------------------------------------------------
    # 3) Jump risk (Parkinson-only, thresholded surprise)
    # sigmahat_pre[t] = sqrt( mean_{i=1..m} sigma^2_{t-i} )
    # jump[t] = 1(|logret_t| > k * sigmahat_pre[t])
    #
    # IMPORTANT: baseline uses ONLY past data within the file
    # --------------------------------------------------------
    ...
    out["sigmahat_pre_m"] = sigmahat_pre  # debug / inspection
