...
CAP_DAYS = 5  # maximum forward-fill length
    ...
    for i in range(len(out)):
        ...
        if pd.isna(val):
            ...
            if not pd.isna(last_val) and run_len <= cap:
                out.iat[i] = last_val  # carry forward
            else:
                out.iat[i] = 0.0       # too long / no last value -> neutral
        else:
            # valid value resets the run
            last_val = val
            run_len = 0
...
def main():
    ...
    # Identify channels from low_evidence_ columns
    ...
    # Channel name extraction: low_evidence_<channel> corresponds to z_<channel>
    channels = [c.replace("low_evidence_", "") for c in low_cols]

    # ------------------------------------------------------------
    # 1) ONLY replace z_<channel> NaNs with 0 when low_evidence==1
    # ------------------------------------------------------------
    for ch in channels:
        zcol = f"z_{ch}"
        ...
        if zcol not in df.columns:
            # If some channel has no z_ column, skip gracefully
            continue

        # Make sure z column is numeric; blanks become NaN
    ...
    # ------------------------------------------------------------
    # 2) LOCF imputation with cap=5 for remaining NaNs (true missing)
    #    Apply only to z_ columns
    # ------------------------------------------------------------
    for ch in channels:
        zcol = f"z_{ch}"
        if zcol not in df.columns:
