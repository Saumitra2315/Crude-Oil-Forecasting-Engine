...
# Root folder that contains: 2017, 2018, ..., 2026 (each year -> month folders -> day csvs)
...
# If a file exists but is smaller than this, treat it as missing/bad
MIN_SIZE_KB = 10
MIN_SIZE_BYTES = MIN_SIZE_KB * 1024

# If True, we will consider both ".csv" and ".xlsx" extensions when checking existence.
# (Your screenshot shows Excel icons but names look like CSV stems; Windows can show CSV as Excel.)
CHECK_BOTH_CSV_XLSX = True

# Output report file (optional). Set to None to skip writing.
...
def find_file_for_day(year_dir: Path, day: date) -> Tuple[Optional[Path], int]:
    ...
    # Fast-ish: search by glob pattern. (rglob on year dir is fine for 10 years)
    # Match both csv and xlsx optionally.
    patterns = [f"*{ymd}*.csv"]
    ...
    # Pick the largest (in case duplicates exist)
    best = None
    best_size = -1
    for p in candidates:
        try:
...
def main():
    ...
    for y in YEARS:
        ...
        if not year_dir.exists():
            # If entire year folder missing, everything is missing
        ...
        for d in days:
            ...
            if size < MIN_SIZE_BYTES:
                missing += 1
                # Include actual found path for debugging
    ...
    # Sort missing entries by date (ascending)
