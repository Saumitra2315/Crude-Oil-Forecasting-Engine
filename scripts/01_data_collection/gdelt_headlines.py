...
# ============================================================
KEYWORDS = "crude oil"         # <-- change keywords here (e.g., 'crude oil OR brent OR wti')
LANGUAGE = "eng"               # <-- change language here (eng = English)
START_DATE = date(2026, 1, 26) # <-- change start date here (YYYY, M, D)
END_DATE   = date(2026, 1, 27) # <-- change end date here (YYYY, M, D)
MAXRECORDS = 75                # <-- default 75; can go up to 250 for artlist
SORT       = "hybridrel"       # <-- e.g., hybridrel, datedesc, dateasc, tonedesc, toneasc
# ============================================================
...
def main() -> None:
    ...
    # output filename includes keyword + timeframe
    ...
    # Minimal sanity prints (won’t crash even if schema changes)
    articles = None
    if isinstance(data, dict):
        # common case: {"articles":[...], ...}
