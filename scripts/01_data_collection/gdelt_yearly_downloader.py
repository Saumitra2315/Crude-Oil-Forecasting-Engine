...
BASE_FOLDER = Path(r"D:\ACADS\4-2\FIN SOP\Data\headlines data")   # <-- change if needed
...
# Patch mode timing (requested: 8 seconds)
...
def ensure_summary_csv(summary_path: Path) -> None:
    ...
    if not summary_path.exists():
        with summary_path.open("w", newline="", encoding="utf-8") as f:
            ...
            w.writerow([
                "date",
                "status",          # SUCCESS / FAIL / SKIP / RETRY_SUCCESS / RETRY_FAIL
                "http_status",
                "bytes_written",
                "file_path",
                "error"
...
# ============================================================
# ✅✅✅ PATCH MODE (RETRY ONLY MISSING + FAILED, IGNORE CORRUPT) ✅✅✅
# ============================================================
...
def is_valid_gdelt_csv_bytes(head_bytes: bytes) -> Tuple[bool, str]:
    ...
    # Typical GDELT CSV header contains URL and Title columns
...
def safe_download_csv_patch(url: str, out_path: Path, *, timeout: int = 90) -> Tuple[int, int, bool, str]:
    ...
    if not ok:
        # Do NOT overwrite existing file with junk
        try:
...
def scan_year_for_patch(year: int) -> Dict[int, Dict[str, List[date]]]:
    ...
    # Use BOTH summaries (main + prior patch) so "latest status" is accurate
...
def patch_one_year(year: int) -> None:
    ...
    for m in range(1, 13):
        ...
        corrupt_n = len(stats[m]["corrupt"])   # report only
