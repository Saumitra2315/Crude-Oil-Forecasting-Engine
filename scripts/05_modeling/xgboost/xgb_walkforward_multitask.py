...
RESULTS_DIR = ARTIFACTS_DIR / "results" / "xgboost_walkforward"
...
REGRESSION_ESTIMATORS = 3000
CLASSIFICATION_ESTIMATORS = 2500
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run walk-forward XGBoost experiments for return, volatility, and jump targets "
            "using repo-native artifacts plus channel ablations."
        )
    )
    parser.add_argument(
        "--tasks",
...
def _load_channels_full() -> pd.DataFrame:
    ...
    numeric_cols = [c for c in full.columns if c.startswith("mass_") or c.startswith("num_") or c.startswith("z_") or c.startswith("low_evidence_")]
...
def _build_channel_feature_frame(channels: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Dict[str, str]]]:
    ...
    for zcol in CHANNEL_COLS:
        ...
        low_evidence = pd.to_numeric(work.get(lecol), errors="coerce").fillna(0).astype(int) if lecol in work.columns else pd.Series(0, index=work.index)
...
def _scenario_definitions(scenario_set: str, variant_maps: Dict[str, Dict[str, str]]) -> List[Scenario]:
    ...
    single_channel_runs = {f"single__{re.sub('[^A-Za-z0-9]+', '_', c.replace('z_', '')).strip('_').lower()}": [c] for c in CHANNEL_COLS}
    ...
    for variant_name, mapping in variant_maps.items():
        if include_core:
            for name, base_channels in core_channels.items():
                ...
                if name != "market_only":
                    features += [GLOBAL_SENTIMENT_COL] if name == "market_plus_global_sentiment" else []
                if name in {"market_plus_top3_channels", "market_plus_all6_channels"}:
                    features += [mapping[c] for c in base_channels]
                    notes.append(f"channels={len(base_channels)}")
                elif name == "market_plus_global_sentiment":
                    notes.append("global_sentiment_only")
                else:
                    notes.append("no_news_features")

                scenarios.append(Scenario(name=f"{variant_name}__{name}", channel_variant=variant_name, feature_cols=features, notes=", ".join(notes)))
        ...
        if include_ablation:
            for name, base_channels in single_channel_runs.items():
                ...
                scenarios.append(Scenario(name=f"{variant_name}__{name}", channel_variant=variant_name, feature_cols=features, notes=notes))
...
def _build_params(task: TaskConfig, y_train: np.ndarray) -> Tuple[Dict[str, Any], int]:
    if task.task_type == "regression":
        params = dict(REGRESSION_PARAMS)
        params["objective"] = "reg:squarederror"
        params["eval_metric"] = "rmse"
        return params, REGRESSION_ESTIMATORS

    params = dict(CLASSIFICATION_PARAMS)
    ...
    return params, CLASSIFICATION_ESTIMATORS
...
def _train_booster(
    ...
    params, n_estimators = _build_params(task, y_train)

    def _fit(params_local: Dict[str, Any]) -> xgb.Booster:
        return xgb.train(
            params=params_local,
            dtrain=dtrain,
    ...
    if USE_GPU:
        if _xgb_supports_device_param():
            try:
                ...
                print("[WARN] GPU training (device=cuda) failed, falling back. Reason:", exc)
        ...
        try:
            ...
            print("[WARN] Legacy GPU training failed, falling back to CPU. Reason:", exc)
...
def _predict_booster(booster: xgb.Booster, X: np.ndarray, feature_names: List[str]) -> np.ndarray:
    ...
    try:
        ...
        if best_ntree_limit is not None and int(best_ntree_limit) > 0:
            return booster.predict(dmat, ntree_limit=int(best_ntree_limit))
    except Exception:
        pass
...
def _prepare_task_frames(df: pd.DataFrame, task: TaskConfig, scenario: Scenario) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    feature_fill_cols = [c for c in scenario.feature_cols if c == GLOBAL_SENTIMENT_COL]
...
def _walk_forward_backtest(
    ...
    for fold_idx, start in enumerate(range(0, len(test_df), test_step_rows), start=1):
        ...
        booster, used_gpu, best = _train_booster(task, X_train, y_train, X_val, y_val, feature_cols)
        used_gpu_any = used_gpu_any or used_gpu
        raw_pred = _predict_booster(booster, X_block, feature_cols)
...
def _fit_reference_model(train_df: pd.DataFrame, task: TaskConfig, feature_cols: List[str]) -> Tuple[xgb.Booster, bool, Dict[str, float], pd.DataFrame, pd.DataFrame]:
    ...
    booster, used_gpu, best = _train_booster(task, X_train, y_train, X_val, y_val, feature_cols)
    return booster, used_gpu, best, fit_df, val_df
...
def _shap_outputs(
    ...
    if task.task_type != "regression":
        return
    if not _HAS_SHAP:
        print("[WARN] SHAP not installed. Skipping SHAP outputs.")
        return
    ...
    shap_vals = np.asarray(shap_vals, dtype=float) / task.scale

    expected = explainer.expected_value
    base_value = float(expected) if np.isscalar(expected) else float(expected[0])
    base_value = base_value / task.scale

    safe_name = re.sub(r"[^A-Za-z0-9_-]+", "_", f"{task.name}__{scenario.name}").strip("_")
    ...
    pred_scaled = _predict_booster(booster, te.to_numpy(), feature_cols)
    ...
    meta["y_pred"] = pred_scaled / task.scale
...
def _select_shap_scenarios(scenarios: List[Scenario]) -> set[str]:
    preferred = {
...
def main() -> None:
    ...
    shap_targets = _select_shap_scenarios(scenarios)
    ...
    for task_name in args.tasks:
        ...
        for scenario in scenarios:
            print("=" * 80)
            print(f"[RUN] task={task.name} scenario={scenario.name}")
            train_df, test_df = _prepare_task_frames(full_df, task, scenario)
            feature_cols = task.lag_feature_cols + scenario.feature_cols

            if len(test_df) == 0:
                print(f"[WARN] Skipping task={task.name} scenario={scenario.name}: no usable test rows")
                continue
            ...
            ref_booster, ref_used_gpu, ref_best, fit_df, val_df = _fit_reference_model(train_df, task, feature_cols)
            ref_path = outdir / f"reference_model__{safe_name}.json"
            ref_booster.save_model(ref_path)

            if (not args.disable_shap) and (scenario.name in shap_targets):
                _shap_outputs(outdir, task, scenario, ref_booster, fit_df, test_df, feature_cols)
    ...
    if not summary_rows:
        raise RuntimeError("No experiments completed. Check input artifacts and scenario/task filters.")
    ...
    print("[OK] Saved walk-forward outputs to:", outdir)
