...
TARGET_COL = "Return"  # already computed by you

# Exogenous variable(s)
EXOG_COLS: List[str] = ["sentiment_t_minus_1"]

# IMPORTANT: your file already contains t-1 sentiment aligned to day t
SHIFT_EXOG_BY_1 = False

# IMPORTANT: with exog + extend() 1-step updates, trend must be "n"
# We add intercept manually as an exog column.
...
def _prep_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # strict DD-MM-YYYY first (your files)
...
def _walk_forward_predict(res, y_test: np.ndarray, exog_test: Optional[np.ndarray]) -> np.ndarray:
    ...
    for i in range(len(y_test)):
        ...
        # update state with realized y_t
...
def main():
    ...
    # Build exog list (optionally add intercept)
    ...
    # Optional extra shifting (OFF for your t-1 file)
